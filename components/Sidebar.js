...
export default function Sidebar({ isOpen, onClose }) {
  ...
  // Updated function to correctly match active links
  const linkClasses = (href) => {
    // Exact match for /admin (Dashboard), so it doesn't get active on nested routes
    if (href === "/admin") {
    ...
    // For other links, check if pathname is exact or starts with href + "/"
    const hrefWithSlash = href.endsWith("/") ? href : href + "/";
    return pathname === href || pathname.startsWith(hrefWithSlash)
      ? "w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left bg-[#144A6C] text-white font-normal pl-6 [&>svg]:w-6 [&>svg]:h-6"
      : "w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left text-gray-700 hover:bg-gray-100 transition-colors";
  };
  ...
  const sidebarItems = [
    ...
    { icon: <Users className="w-5 h-5" />, label: "Doctors", href: "/admin/doctors" },
    {
  ...
  return (
    <>
      ...
      <div
        className={`fixed lg:static inset-y-0 left-0 z-50 lg:z-auto md:bg-[#CEE5E433]  bg-white 
      ...
      >
        <div className="mb-10 mt-4">
          <h2 className="text-2xl font-regular text-[#144A6C]">Welcome Back !</h2>
        </div>
        ...
        <div className="mt-8 space-y-1 flex-shrink-0">
          <Link href="/admin/support" onClick={onClose} className={linkClasses("/admin/support")}>
            <HelpCircle className="w-5 h-5" />
            <span className="font-medium">Support</span>
          </Link>

          <Link href="/auth/Login" onClick={onClose} className={linkClasses("/auth/Login")}>
          ...
          <div className="bg-[#CEE5E4] rounded-lg p-4 mt-4 flex-shrink-0">
            ...
            <Link href="/upgrade" onClick={onClose} className="text-[#144A6C] font-medium text-1xl">
              Upgrade plan
            </Link>
          </div>
        </div>
